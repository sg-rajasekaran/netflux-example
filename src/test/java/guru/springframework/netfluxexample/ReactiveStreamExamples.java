...
public class ReactiveStreamExamples {
    ...
    public void filterAndSortStreamExample() throws Exception {
        ...
        dogs.filter(s -> s.length() == 6).take(3).sort().subscribe(System.out::println);
    }
    ...
    public void filterAndSortStreamWithCollectorExample() throws Exception {
        ...
        dogs.filter(s -> s.length() <= 6).take(4).sort().collect(Collectors.joining(", ")).subscribe(System.out::println);
    }

    @Test
    public void testFlatMap() throws Exception {
        Flux<List<List<Integer>>> listFlux = Flux.just(Arrays.asList(Arrays.asList(1, 2, 3), Arrays.asList(4, 5, 6)));
        listFlux.flatMap(lists -> Flux.fromIterable(lists)).flatMap(lists -> Flux.fromIterable(lists)).subscribe(System.out::println);
    }
